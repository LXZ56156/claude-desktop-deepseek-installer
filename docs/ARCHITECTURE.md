...
同一领域模块现在还提供纯 redirect/final-header normalization contract。瞬态
24 字段 facts 固定一次 GET 307 → GET 200、唯一 Location、redirect 的受检 header
形状、终点无下一跳、无参数 `application/octet-stream`、无 encoding/chunking 和
64 bytes–1 GiB 唯一 Content-Length。raw `downloads.claude.ai` transport URI 可在
调用栈内携带不透明 query，但 query 前的 raw URI 必须逐字节等于 canonical
`https://downloads.claude.ai/releases/win32/x64/<numeric.version>/Claude-<40-lower-hex>.msix`
projection；禁止 URI canonicalization 改写 path/authority，Unicode control/separator
也 fail closed。转换器只返回这个 scheme/host/path projection；raw URI/query 不进入
19 字段安全 observation、binding、异常、receipt 或任何持久面。文件名中的 40 位
hex 只作为已核验 routing grammar，不被当作 body hash。这个纯 observation 只证明
传入 claim 的结构与绑定，不执行 HTTP，也不是 body、artifact 或 publisher evidence。
...
因其返回前已
关闭句柄，不能直接作为公开 `Save` handoff；也没有 connected network producer、
可信 header extractor、原子提交、receipt/evidence bundle 或 installer consumer。
source descriptor 的 Anthropic SHA/signer/publisher/identity 仍未冻结，
verify-to-install 的重新锁定与执行前重验也未实现，因此当前没有可传给安装或发布门的
authoritative 正向结果。

公开 `Save-CddsiOfficialClaudeDesktopMsix` 仍是无 I/O plan-only 入口，并未消费上述
pure observation 或 acquisition session；它不再把 mutable `latest` descriptor token
宣称为 cache key。真正接线还必须新增 held-directory identity、owner/DACL/trusted-writer
证明、显式 snapshot receipt binding，并让 downloader-owned `CreateNew` 文件 handle
连续存活到 hash/flush/manifest/WVT/receipt 完成，不能依赖 close/move/reopen 后的路径
重开。
