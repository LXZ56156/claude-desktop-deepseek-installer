...
> 现有入口只会返回 `scaffold_only`。D-027 正式只支持 Windows 11 x64，并以
> Windows PowerShell 5.1 运行。Git 静默安装、Claude 安装、配置和三个界面尚未真实
> 通过。本文件描述未来 D-027 候选在 clean disposable VM snapshot 验收后必须遵守的
> 用户合同，不代表当前代码可安装。

## 使用前

- D-027 只支持 Windows 11 x64；Windows 10、Arm64 和其他 Windows 版本不在首版
  支持或验收范围。
